            ...
            @foreach ($plant->controllers as $controller)
                <div class="mb-6 border-2 border-indigo-200 rounded-lg p-4 bg-indigo-50">
                    <div class="mb-2 flex items-center justify-between">
                        ...
                        <span class="text-xs text-gray-500">UUID: {{ $controller->uid ?? $controller->id ?? 'N/A' }}</span>
                    </div>
                            ...
                            <div class="mb-4 border rounded p-3 bg-white">
                                ...
                                <div class="overflow-x-auto">
                                    <table class="min-w-full table-fixed text-sm border rounded">
                                        ...
                                        <tbody>
                                        @php
                                            $parents = collect($feed->devices ?? [])->where('parent_device', true);
                                        @endphp
                                        @foreach ($parents as $parent)
                                            <tbody x-data="{ open: false }" class="bg-gray-100 hover:bg-indigo-100 transition">
                                            <tr>
                                                <td class="text-center">
                                                    <button type="button" class="focus:outline-none" @click.stop="open = !open" :aria-expanded="open" aria-label="Toggle children">
                                                        <template x-if="!open">
                                            ...
                                            @php
                                                $slaveRows = collect($feed->devices ?? [])->where('parent_device', false)->where('parent_device_id', $parent->id)->values();
                                            @endphp
                                            @foreach ($slaveRows as $i => $child)
                                                <tr x-show="open" x-transition class="{{ $i % 2 === 0 ? 'bg-indigo-50' : 'bg-indigo-100' }} hover:bg-indigo-200 cursor-pointer transition">
                                                    <td></td>
