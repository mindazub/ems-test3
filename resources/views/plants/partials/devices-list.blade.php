        ...
        @foreach ($plant->controllers as $controller)
            <div class="mb-6 border-2 border-indigo-200 rounded-lg p-4 bg-indigo-50">
                <div class="mb-2 flex items-center justify-between">
                    <h4 class="text-xl font-bold text-indigo-800">Controller #{{ $controller->uid }} <span class="text-xs text-gray-500">( CONTROLLER)</span></h4>
                    <span class="text-xs text-gray-500">UUID: {{ $controller->uid }}</span>
                </div>
                @if (!empty($controller->mainFeeds) && is_iterable($controller->mainFeeds))
